...
@Keep
 public class TypedValueHolder {
    ...
    private static ArrayMap<Long, Object> sHolder = new ArrayMap<>();
      ...
      static long putValue(Object object) {
        long time = -1;
        if (object == null)
    ...
    @SuppressWarnings("unchecked")
      static <E> E getAndRemoveValue(long key, Class<E> pEClass) {
    ...
    @SuppressWarnings("unchecked")
      static <E> long save(E pE, String saveKey, int type, Bundle outstate) {
        checkType(type);
        long key = 0;
        try {
            switch (type) {
    ...
    @SuppressWarnings("unchecked")
      static <E> E getAndRemoveValue(long key, Class<E> pE, String saveKey, int type, Bundle outstate) {
        checkType(type);
        E e = null;
        try {
            switch (type) {
